...
function frameThree() {
	...
	.from(['#f4-bg'], { duration: 0.7, stagger: 0.1, opacity: 0, x:160 })
...
function frameFour() {
	...
	.from(['#f5-bg'], { duration: 0.5, stagger: 0.8, opacity: 0 })//Women
	...
	gsap.delayedCall(1.9, frameFive);
...
function frameFive() {
	...
	gsap.delayedCall(delayFrame, frameSix);
...
function frameSix() {
	...
	const tl = gsap.timeline({});
	tl
		.from(['#f7-bg'],  { duration: 1.5, stagger: 0.1, opacity: 0, scale: 1.1 })
